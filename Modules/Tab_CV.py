...
class CVTab(GenericTab):
    ...
    def __init__(self, parent):
        ...
        self.VBox  = wx.ComboBox(self, value='VSU1', pos=(X+3*Margin+5, Y+2*Margin), size=(60,40), choices=['SMU1','SMU2','SMU3','SMU4', 'VSU1', 'VSU2'], name='sv_VBox')
        self.Sizer1a.Add(self.VBox)

        self.CompTx = wx.StaticText(self, label='Imax:', pos=(X+1*Margin+SMU_MarginX, Y+2*Margin+3))
        self.Comp  = wx.ComboBox(self, value='10m', pos=(X+4*Margin+5+SMU_MarginX, Y+2*Margin), size=(60,40), name='sv_Imax')

        self.CBoxTx = wx.StaticText(self, label='C:', pos=(X+1*Margin, Y+2*Margin+3+SMU_MarginY))
        self.CBox = wx.ComboBox(self, value='VMU1', pos=(X+3*Margin+5, Y+2*Margin+SMU_MarginY), size=(60,40), choices=['VMU1', 'VMU2'], name='sv_CBox')
        self.Sizer1a.Add(self.CBox)

        self.SBoxTx = wx.StaticText(self, label='S:', pos=(X+2*Margin+SMU_MarginX,  Y+2*Margin+3+SMU_MarginY))
        self.SBox = wx.ComboBox(self, value='', pos=(X+4*Margin+5+SMU_MarginX, Y+2*Margin+SMU_MarginY), size=(60,40), choices=['VMU1', 'VMU2'], name='sv_SBox')
        ...
        self.VStop= wx.SpinCtrlDouble(self, value=str (DefaultMax), pos=(BoxVgs[0][0]+2*Margin, BoxVgs[0][1]+50), size=(80,35), min=-100, max=100, inc=0.5,style=wx.SP_ARROW_KEYS|wx.TE_CENTRE, name='sv_VStop')
        ...
        self.VStart = wx.SpinCtrlDouble(self, value=str (-DefaultMax), pos=(BoxVgs[0][0]+2*Margin, BoxVgs[0][1]+50+70), size=(80,35), min=-100, max=100, inc=0.5, style=wx.SP_ARROW_KEYS|wx.TE_CENTRE, name='sv_VStart')
        ...
        self.VPoints= wx.SpinCtrlDouble(self, value=str (200), pos=(BoxVgs[0][0]+2*Margin, BoxVgs[0][1]+50+140), min=10, max=2000, inc=10, size=(80,35), style=wx.SP_ARROW_KEYS|wx.TE_CENTRE, name='sv_VSteop')
        ...
        self.IntTimeTx = wx.StaticText(self, label='Integration Time:', pos=(BoxVgs[0][0],BoxVgs[0][1]+BoxVgs[1][1]+Margin-10))
        self.IntTimeBox = wx.ComboBox(self, value=DefaultIntTime, pos=(BoxVgs[0][0],BoxVgs[0][1]+BoxVgs[1][1]+Margin+10), size=(80,20), choices=['SHORt','MEDium','LONG'], style=wx.CB_READONLY, name='sv_IntTime')
        self.ScaleTx= wx.StaticText(self, label='Scale:', pos=(BoxVgs[0][0],BoxVgs[0][1]+BoxVgs[1][1]+Margin+35))
        self.ScaleBox = wx.ComboBox(self, value='2 nF', pos=(BoxVgs[0][0],BoxVgs[0][1]+BoxVgs[1][1]+Margin+55), size=(80,20), choices=['2 nF','200 pF','20 pF/2 pF'], style=wx.CB_READONLY, name='sv_Scale')
        self.HTimeTx = wx.StaticText(self, label='Hold Time (s):', pos=(BoxVgs[0][0],BoxVgs[0][1]+BoxVgs[1][1]+Margin+80))
        self.HTimeBox = wx.SpinCtrlDouble(self, value='1', pos=(BoxVgs[0][0],BoxVgs[0][1]+BoxVgs[1][1]+Margin+100), size=(80,20), min=0, max=10, inc=0.1, style=wx.SP_ARROW_KEYS|wx.TE_CENTRE, name='sv_HoldTime')
